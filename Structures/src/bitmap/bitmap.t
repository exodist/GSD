...
int main() {
    ...
    assert( bitmap_fetch(b, 1) == 0 );
    assert( bitmap_get(b, 0) == 1 );

    assert( bitmap_fetch(b, 1) == 1 );
    assert( bitmap_get(b, 1) == 1 );

    assert( bitmap_fetch(b, 1) == 3 );
    assert( bitmap_get(b, 3) == 1 );

    assert( bitmap_fetch(b, 1) == 4 );
    ...
    assert( bitmap_fetch(b2, 1) == 5 );
    assert( bitmap_fetch(b2, 1) == -1 );

    bitmap *b3 = bitmap_clone(b2, 1024);
    assert( bitmap_fetch(b3, 1) == 6 );
    ...
    for (int i = 0; i < 1024; i++) {
        ...
        assert( bitmap_fetch(b4, 1) == i );
        assert( bitmap_get(b4, i)   == 1 );
    }
    for (int i = 0; i < 1024; i++) {
        assert( bitmap_get(b4, i)   == 1 );
        assert( bitmap_fetch(b4, 0) == i );
        assert( bitmap_get(b4, i)   == 0 );
    }
